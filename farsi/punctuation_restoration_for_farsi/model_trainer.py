...
class ModelTrainer():
    def __init__(self, task:int, model:str,run_name:str, data_percentage:float,use_token_type_ids:bool, opimizer_config, tokenizer_config,languages,do_hyperparameter_search = False, **args):
        ...
        print(f"Run Name: {run_name} (type: {type(run_name)})")
        print(f"Data Percentage: {data_percentage} (type: {type(data_percentage)})")
        print(f"Optimizer Config: {opimizer_config} (type: {type(opimizer_config)})")
        print(f"Tokenizer Config: {tokenizer_config} (type: {type(tokenizer_config)})")
        print(f"Languages: {languages} (type: {type(languages)})")
        print(f"Use Token Type IDs: {use_token_type_ids} (type: {type(use_token_type_ids)})")
        print(f"Do Hyperparameter Search: {do_hyperparameter_search} (type: {type(do_hyperparameter_search)})")
        self.task = task 
    ...
    def compute_metrics_generator(self):
        def metrics(pred):
            ...
            print(f"Original labels: {pred.label_ids}")
            print(f"Original predictions: {pred.predictions}")
            ...
            if self.task == 1:
                ...
                # Debugging print statements for classification report
                print("\n----- report -----\n")
                print(f"Label-to-ID mapping: {self.label_2_id}")
                print(f"Keys used for target_names: {self.label_2_id.keys()}")
                try:
                ...
                # Debugging print statements for confusion matrix
                print("\n----- confusion matrix -----\n")
                ...
                try:
                    ...
                    print(f"Error while printing confusion matrix: {e}")
    ...
    def run_training(self):
        ...
        ## train model
        print(f"Optimizer Config: {self.opimizer_config}")
